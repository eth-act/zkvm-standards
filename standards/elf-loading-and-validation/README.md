...
A concrete case is the RISC-V **large code model** (`-mcmodel=large`), under which both major toolchains place read-only data in the executable section by design. Because read-only data may then lie outside the ±2 GiB PC-relative reach of a code-relative access, it has to sit adjacent to the code it serves: GCC (from GCC 15) emits switch jump tables into `.text` in this model, and LLVM is moving integer/floating-point constant pools the same way. This is not a defect but an addressing requirement of the model, which is why no compiler option removes it. The medium code models used by current guests keep read-only data in dedicated data sections — but that behavior is model-dependent and version-dependent, so nothing in the toolchain's contract prevents a different model, or a future release, from doing otherwise.
