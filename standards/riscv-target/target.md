...
Supporting unaligned accesses in zkVMs is believed to be implementable efficiently without performance penalties for the common case. zkVM implementations can detect whether unaligned accesses occur during execution and only incur additional proving costs when they are actually used. Programs that never perform unaligned accesses - which represents the vast majority of correctly-compiled code - would experience no overhead. This design approach makes `Zicclsm` support effectively "free" for well-behaved programs while providing a critical safety net for edge cases.
...
As a side benefit, unaligned access support may provide performance advantages for certain workloads. Deserialization of binary protocols, a common operation in zkVM contexts, can be faster on architectures that support unaligned accesses, as evidenced by the Linux network stack on x86_64 and aarch64. The Rust compiler can be configured to target RISC-V implementations with unaligned access support, enabling it to emit more efficient deserialization code. While we currently lack empirical data demonstrating that proving unaligned accesses in zkVMs provides net performance benefits in these scenarios, the potential exists for future optimization opportunities.
